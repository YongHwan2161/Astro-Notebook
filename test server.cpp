function render3DModel(canvas, objData, mtlData, textureData, isUrl = false) {
            ...
            return new Promise((resolve, reject) => {
                if (isUrl) {
                    textureLoader.load(
                        texture.url || texture,
                        (loadedTexture) => {
                            console.log(`Texture ${texture.name} loaded successfully:`, loadedTexture);
                            resolve({ name: texture.name, texture: loadedTexture });
                        },
                        undefined,
                        (error) => {
                            console.error(`Failed to load texture ${texture.name}: ${error.message}`);
                            resolve({ name: texture.name, texture: null });
                        }
                    );
                } else {
        ...
        object.traverse((child) => {
            if (child instanceof THREE.Mesh) {
                ...
                textures.forEach(tex => {
                    if (tex.texture) {
                        switch (tex.name.toLowerCase()) {
                            case 'cat_diffuse.jpg':
                                material.map = tex.texture;
                                break;
                            case 'cat_bump.jpg':
                                material.bumpMap = tex.texture;
                                break;
                            // Add more cases for other texture types if needed
                        }
                        console.log(`Applied ${tex.name} to material:`, material.name);
                    }
                });
